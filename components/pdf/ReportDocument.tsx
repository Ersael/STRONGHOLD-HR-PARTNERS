      ...
      <Page size="A4" style={styles.page} wrap>
        ...
        {report.partial && (
          <View style={styles.partialBanner}>
            <Text style={styles.partialBannerTitle}>Resultados parciales</Text>
            <Text style={styles.partialBannerText}>
              Pruebas completadas: {report.completed_assessments.length > 0
                ? report.completed_assessments.map(batteryName).join(", ")
                : "ninguna"}
              . Pruebas pendientes: {report.pending_assessments.map(batteryName).join(", ")}. Las secciones
              que dependen de una batería pendiente lo indican explícitamente en vez de mostrar contenido
              genérico.
            </Text>
          </View>
        )}
