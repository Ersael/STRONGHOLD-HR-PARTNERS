...
export function QuestionRenderer({
  ...
  saving: boolean;
  /**
   * true cuando la prueba ya no está 'in_progress' (completada, expirada,
   * etc.) y los controles deben verse pero no aceptar interacción. Usado
   * por el portal nuevo por sesión (Fase A) al reabrir una pregunta ya
   * cerrada; el portal viejo por token no lo pasa (default false, mismo
  ...
  if (question.question_type === "likert5") {
    return <Likert5Question question={question} onAnswer={onAnswer} saving={isDisabled} />;
  }
  if (question.question_type === "multiple_choice") {
    return <MultipleChoiceQuestion question={question} onAnswer={onAnswer} saving={isDisabled} />;
  }
  return <ForcedChoiceQuadQuestion question={question} onAnswer={onAnswer} saving={isDisabled} />;
}
