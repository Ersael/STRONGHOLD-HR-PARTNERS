...
import type { AssessmentCode, AssessmentConfigJson, Database } from "@/types/database";
...
/**
 * Determina, para un candidato, qué baterías (códigos de
 * `assessment_definitions`) tienen AL MENOS UNA `candidate_assessment`
 * 'completed' ("completed") y cuáles fueron asignadas pero NO tienen
 * ninguna fila 'completed' todavía ("pending" — agrupa `pending`,
 * `in_progress`, `expired` y `revoked`, porque ninguno de esos estados
 * produce scores utilizables para el reporte).
 *
 * Fase B: usado por `generateReport()` (vía `app/api/reports/[candidateId]`)
 * para poder marcar el reporte como `partial` y para que las secciones que
 * dependen de una sola batería completa (ver lib/report/sections.ts) digan
 * explícitamente que no se pueden generar todavía, en vez de rellenar con
 * contenido genérico.
 *
 * Nota: si una batería tiene varias filas (ej. re-asignada tras revocar la
 * primera) y AL MENOS UNA está 'completed', se cuenta como completada
 * aunque otra fila de la misma batería esté en otro estado.
 */
export async function fetchAssessmentCompletionStatus(
  supabase: SupabaseClient<Database>,
  candidateId: string,
): Promise<{ completed: AssessmentCode[]; pending: AssessmentCode[] }> {
  const { data: candidateAssessments, error } = await supabase
    .from("candidate_assessments")
  ...
  const definitionIds = Array.from(new Set(candidateAssessments.map((ca) => ca.assessment_definition_id)));
  const { data: definitions, error: defError } = await supabase
    .from("assessment_definitions")
    .select("id, code")
    .in("id", definitionIds);
  ...
  if (defError) throw defError;
  const codeByDefinitionId = new Map((definitions ?? []).map((d) => [d.id, d.code]));

  const completedCodes = new Set<AssessmentCode>();
  const anyStatusCodes = new Set<AssessmentCode>();

  for (const ca of candidateAssessments) {
    const code = codeByDefinitionId.get(ca.assessment_definition_id);
    if (!code) continue;
    anyStatusCodes.add(code);
    if (ca.status === "completed") completedCodes.add(code);
  }

  const pendingCodes = Array.from(anyStatusCodes).filter((code) => !completedCodes.has(code));

  return { completed: Array.from(completedCodes), pending: pendingCodes };
}
